...
// the backend's `secureMode` flag (null when unknown). Any failure (thrown error
// or non-OK backend response) is reported as offline rather than surfacing a 500.
export async function GET(request: NextRequest) {
  const debug = request.nextUrl.searchParams.get("debug") === "1";
  try {
    const { ok, secureMode } = await checkHeartbeat({ debug });
    return NextResponse.json(
      { status: ok ? "online" : "offline", secureMode },
      { status: ok ? 200 : 503 },
    );
  } catch {
    return NextResponse.json(
      { status: "offline", secureMode: null },
      { status: 503 },
    );
  }
}
