...
export default function VideoGameBoxesManager({
  ...
  const [sorts, setSorts, initialResolvedSorts] =
    usePersistentSorts("video-game-box", initialSorts);
  ...
  const lastDto = useRef(
    JSON.stringify(toFilterRequest("videoGameBox", initialFilters)),
  );
      ...
      .then(
        async ([
          ...
          const seeded = supportsSorting(loadedSpec)
            ...
            : [];
          const dto = [
            ...toFilterRequest("videoGameBox", initialResolvedFilters),
            ...toSortRequest(
              "videoGameBox",
