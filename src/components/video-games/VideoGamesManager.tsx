...
// server-side). An empty filter set returns every video game.
async function searchVideoGamesClient(
...
// All systems, for the System cell's dropdown and the system_id filter's
// name list. Loaded once on mount alongside the games.
async function searchSystemsClient(signal?: AbortSignal): Promise<System[]> {
...
// Video game custom-field definitions, reusing the existing custom-fields
// route. They give the authoritative ordered column set for the dynamic columns.
async function fetchVideoGameFields(
  signal?: AbortSignal,
): Promise<CustomField[]> {
  const res = await fetch("/api/custom-fields/entity/videoGame", { signal });
  const data = await readJson<CustomField[]>(res);
  return [...data].sort((a, b) => a.order - b.order);
}

// The video game filter spec (standard filterable fields + their operators).
...
async function fetchFilterSpec(
  ...
  const res = await fetch("/api/filters/videoGame", { signal });
...
export default function VideoGamesManager() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetchVideoGameFields(controller.signal),
